...
public static class LanguageCultureResolver
{
    ...
    {
        if (string.IsNullOrWhiteSpace(languageName)) return CultureInfo.GetCultureInfo("en-US");
        ...
        if (!LooksLikeCultureCode(normalized)) return CultureInfo.GetCultureInfo("en-US");
